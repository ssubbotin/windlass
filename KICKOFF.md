...
**`vllm-service` is stopped and staying stopped** — the owner released the machine and will say when to switch Qwen3.6 back. While it is down the private PR bots have no model. Restore with `sudo systemctl start vllm-service` (it is still `enabled`, so it also returns on reboot).
