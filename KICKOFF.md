...
**`vllm-service` is running again and is in real use.** It was restarted 2026-08-02 20:36 UTC, shortly after this file was first written, and served 10,154 requests in the following week. It holds 92.5 GB of 97.9 GB, so windlass cannot run alongside it — its expert cache alone wants 61 GB. Stop it for a GPU window (`sudo systemctl stop vllm-service`), restart immediately after, and confirm `/v1/models` on :8000 returns 200 before releasing. While it is down the private PR bots have no model, so ask before taking the window.
...
1. **Verify Task 9 on a GPU.** Ask before taking the window — Qwen3.6 is in real use. Then:
   ...
   ssh gpu-box 'sudo systemctl start vllm-service'   # confirm /v1/models on :8000 is 200
   ```
